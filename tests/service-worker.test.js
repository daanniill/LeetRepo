...
const onboardedSettings = { connected: true, owner: "alex-c", repo: "solutions", branch: "main", spacedRepetition: true };
...
function mockRepository(t, item) {
  ...
  t.mock.method(globalThis, "fetch", async (url, init = {}) => {
    ...
    if (target.origin === "https://api.groq.com") {
      const value = {
...
test("GENERATE_FEEDBACK commits direct-provider AI feedback to the tagged GitHub README", async (t) => {
  const item = {
    id: "1-two-sum", number: "1", title: "Two Sum", slug: "two-sum", language: "Python3",
...
test("RATE_REVIEW is blocked while spaced repetition is turned off", async () => {
  globalThis.chrome = createChromeMock({
    local: { submissions: [{ id: "1-two-sum" }], githubAccessToken: "github-token" },
...
test("OPEN_DASHBOARD deep-links onboarded users into the requested view", async () => {
  const mock = createChromeMock({
    local: { githubAccessToken: "github-token" },
    sync: { settings: onboardedSettings }
  });
  globalThis.chrome = mock;
