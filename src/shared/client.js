...
export async function ensureProviderPermission(value) {
  if (!isExtension || !globalThis.chrome?.permissions) return true;
  let origin;
  try {
  ...
  if (!await chrome.permissions.request({ origins: [origin] })) {
    throw new Error("Allow access to the AI endpoint so LeetRepo Lite can send requests to it.");
  }
  return true;
}
...
function demoResponse(type, payload) {
  if (type === "GET_STATE") {
    ...
    return Promise.resolve({
      ...
      ai: {
        ...
        hasApiKey: !onboardingPreview,
        providerLabel: "Groq",
        usage: { date: "2026-08-11", requests: 0, inputTokens: 0, outputTokens: 0, limit: 20 },
        limitReached: false
      }
    });
  }
  ...
  if (type === "CONNECT_GITHUB_TOKEN") return Promise.resolve({ status: "connected", user: { login: "alex-c" }, repos: [{ full_name: "alex-c/leetcode-solutions", name: "leetcode-solutions", owner: { login: "alex-c" }, default_branch: "main" }] });
